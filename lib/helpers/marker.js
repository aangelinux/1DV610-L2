...
export class Marker {
    ...
    _renderPieSlices(data) {
        ...
        data.forEach((object) => {
			...
			const pieText = this.#createPieText(object.name, object.triangleCenter)
            graph.appendChild(pieSlice)
			graph.append(pieText)
            i++
        })
    }
	...
	#createPieText(name, triangleCenter) {
        const text = document.createElementNS("http://www.w3.org/2000/svg", "text")

		text.id = "sliceName"
		text.innerHTML = name
		text.setAttribute("x", triangleCenter.x)
		text.setAttribute("y", triangleCenter.y)
