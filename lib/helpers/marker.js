...
export class Marker {
    ...
    _renderPieSlices(data) {
        ...
        data.forEach((object) => {
            ...
            // const sliceText = this.#createSliceText(object.text, slice)

            graph.appendChild(slice)
            // graph.appendChild(sliceText)
            startCoordinates = this.#updateCoordinates()
            i++
        })
    }
    ...
    #createSliceText(text, slice) {
        ...
        sliceText.setAttribute("x", 0)
        sliceText.setAttribute("y", 0)

        return sliceText
    }

    #updateCoordinates() {
        return this.#scale._updateCoordinates()
    }
