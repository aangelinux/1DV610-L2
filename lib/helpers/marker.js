...
export class Marker {
    ...
    renderLine(data) {
        ...
        const points = this.#createLinePoints(dataPoints)
    ...
    #createDataPoint(dataPoint, i) {
        ...
        circle.setAttribute("cy", 
            this.layout.height - ((dataPoint - this.layout.minValue) * this.layout.heightScale))

        return circle
    }

    #createLinePoints(dataPoints) {
        let points = ""
    ...
    #createPath(sliceData) {
        ...
        // Don't pass the whole coordinates objects, only pass the values
        // the path command doesn't accept strings
        path.setAttribute("d", `
