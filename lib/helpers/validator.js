...
export class Validator {
    ...
    #dataSecondKeyValue = { min: -1000000, max: 1000000 }// TODO need to make separate for different chart types
    ...
    #optionsTypes = {
        ...
        radius: "number",
        title: "string",
        color: "string",  // TODO add method for validating color
        font: "string"  // TODO add method for validating font
    }
    ...
    #validateOptionValues(options) {
        ...
        for (const key in this.#optionsKeysValues) {
            ...
            if (matchingKey) {
                this.#validateRangeOf(options[matchingKey], this.#optionsKeysValues[matchingKey])
            }
        }
    }
    ...
    #validateRangeOf(value, range) {  // TODO Need to tell the user which object is causing the error
        if (!this.#isWithinRange(value, range)) {
            throw new RangeError(`Value of: [${value}] is outside valid range for [${range.min} - ${range.max}]`)
        }
    }

    #validateKey(object, key, schema) {  // TODO See if amount of params can be reduced
